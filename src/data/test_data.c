...
void make_kunden()
{
    ...
    for (loops = (KNDE_DB_RID_BASE+0); loops < (KNDE_DB_RID_BASE+1000); loops ++) 
    {
        ...
        sprintf(k_name,    "Huber%i", loops);
        sprintf(k_vorname, "Sepp%i", loops);
        sprintf(k_strasse, "Hinterweg", loops);
        sprintf(k_plz,     "83000", loops);
        sprintf(k_hnr,     "%i", loops);
        sprintf(k_ort,     "Hinterdupfing%i", loops);
        
        LOG(k_name);      
        strg_in (name,      k_name);
        strg_in (vorname,   k_vorname);
        adrs_in (adresse,   k_strasse, k_hnr, k_plz, k_ort, "D - Germany");
...
void make_konten()
{
    ...
    for (loops=0; loops<2000; loops++)
    {
        ...
        sprintf(k_bankname, "Bank%i", (loops+KNTO_DB_RID_BASE) );
