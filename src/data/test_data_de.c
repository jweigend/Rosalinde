...
void make_kunden()
{
    ...
    for (loops = (KNDE_DB_RID_BASE+0); loops < (KNDE_DB_RID_BASE+1000); loops ++) 
    {
        ...
        sprintf(k_vorname, "Sepp");
        sprintf(k_strasse, "Bahnhofstr.");
        sprintf(k_plz,     "83022", loops);
        sprintf(k_hnr,     "%i", loops);
        sprintf(k_ort,     "Rosenheim%i", loops);
...
void make_konten()
{
    ...
    for (loops=0; loops<2000; loops++)
    {
        ...
        sprintf(k_bankname, "Hypo%i", (loops+KNTO_DB_RID_BASE) );
